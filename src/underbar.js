(function() {
  ...
  _.last = function(array, n) {
    ...
    //setting condition statement - test if n is defined
    //not defined, return the last index value
    if (n === undefined) {
      return array[array.length - 1];
    } else {
    //if it is
    // slice the last values
  ...
  _.each = function(collection, iterator) {
    // detemine collection type
    if (Array.isArray(collection)) {
      // if Array
      // iterate through the collection
      for (var i = 0; i < collection.length; i++) {
        // for each element, call iterator with element, its index, and collection as inputs
        iterator(collection[i], i, collection);
      }
    } else {
      // if Object
      // same thing with object loop
      for (var j in collection) {
        // iterator with element(value), key, and collection as inputs
  ...
  _.filter = function(collection, test) {
    var allTrue = [];
    //want to apply the test to each element in the collection
    //if true
  ...
  _.reduce = function(collection, iterator, accumulator) {
    // edge case
    // collection.length = 1;
    // set a condition based on if there is an accumulator
    ...
    //if no, begin reducing on first element of collection
    //if yes. begin reducing on the accumulator
    //iterate through the collection applying the iterator to each element and the result of the previous iteration
    //return final result
  ...
  _.delay = function(func, wait) {
    ...
    if (arguments.length === 2) {
      ...
      return setTimeout(func, wait, ...args.slice(2));
    }
  };
  ...
  _.shuffle = function(array) {
    ...
    // multiply rand by array length - 1
